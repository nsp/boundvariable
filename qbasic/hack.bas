...
D        IF i > pwdcount THEN GOTO DC
DI       PRINT "guessing pass: " + words(i) + CHR(j) + CHR(k) + CHR(X)
DV       j = XLVIII
DVI      k = XLVIII
DXI      IF CHECKPASS(username, words(i)+CHR(j)+CHR(k)) THEN GOTO DXXVI
DXII     k = k + I
DXIII    IF k < LVIII THEN GOTO DX
DXIV     j = j + I
DXV      IF j < LVIII THEN GOTO DVI
DXX      i = i + I
DXXI     IF i > pwdcount THEN GOTO DC
DXXV     GOTO D
DXXVI    PRINT "found match!!! for user " + username + CHR(X)
DXXVII   PRINT "password: " + words(i) + CHR(j) + CHR(k) + CHR(X)
DXXVIII  END
DC       PRINT "no complex matches for user " + username + CHR(X)
DCI      END
EOF